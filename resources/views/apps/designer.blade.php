...
<div x-data="formDesigner({!! htmlspecialchars(json_encode($app->form_schema ?? ['fields'=>[]]), ENT_QUOTES, 'UTF-8') !!}, {!! htmlspecialchars(json_encode($optionSets->map(fn($o)=>['id'=>$o->id,'code'=>$o->code,'name_th'=>$o->name_th,'name_en'=>$o->name_en])->values()->toArray()), ENT_QUOTES, 'UTF-8') !!})" class="flex gap-4 h-[calc(100vh-10rem)]">
    ...
    <div class="flex-1 bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-4 overflow-y-auto">
        ...
        <div id="field-canvas" x-ref="canvas" class="grid grid-cols-2 gap-3 min-h-[200px] p-2 border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-xl">
            <template x-for="(field, idx) in schema.fields" :key="field.id">
                <div :class="field.width === 'full' ? 'col-span-2' : 'col-span-1'"
                     class="relative p-3 border border-gray-200 dark:border-gray-600 rounded-xl bg-gray-50 dark:bg-gray-750 group cursor-move">
    ...
    <!-- Right: field editor -->
    <div class="w-72 bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-4 overflow-y-auto flex-shrink-0" x-show="editingIdx !== null">
